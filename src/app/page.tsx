    ...
    <main className="max-w-4xl mx-auto p-6">
      <header className="text-center mb-8">
        ...
        <div className="mt-6 flex flex-wrap items-center justify-center gap-3 text-sm">
          {chips.map((k) => (
            <span key={k} className="px-3 py-2 rounded-full bg-black/25 border border-gold">
              {k}
            </span>
          ))}
        </div>
      ...
      <section className="bg-black/25 border border-gold rounded-xl p-4">
        ...
        />
        <div className="mt-4 text-sm opacity-80">Search & JSON-LD inspector will appear here next.</div>
      </section>
    </main>
  );
}
